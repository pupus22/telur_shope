...
function renderHistory(){const rows=[...state.batches].filter(b=>b.status==='active').reverse();$('batchBody').innerHTML=rows.length?rows.map(b=>`<tr><td><b>${esc(b.batchId)}</b></td><td>${esc(dt(b.createdAt))}</td><td><span class="badge ${b.kind==='estimate'?'estimate':'final'}">${esc(b.kind==='estimate'?'Estimasi':b.kind==='mixed'?'Legacy Campuran':'Final')}</span></td><td>${b.items.length}</td><td class="num">${money(b.baseAmount)}</td><td class="num"><b class="${b.correctionAmount>0?'money-positive':b.correctionAmount<0?'money-negative':''}">${b.correctionAmount>0?'+':''}${money(b.correctionAmount)}</b></td><td class="num"><b>${money(b.payoutAmount||b.baseAmount+b.correctionAmount)}</b></td><td><div class="batch-actions"><button class="btn ghost batch-detail" data-id="${esc(b.batchId)}" type="button">Detail</button><button class="btn danger batch-delete" data-id="${esc(b.batchId)}" type="button">Hapus Batch</button></div></td></tr>`).join(''):'<tr><td colspan="8" class="muted">Belum ada Batch.</td></tr>';
  $$('.batch-detail').forEach(b=>b.addEventListener('click',()=>openBatchDetail(b.dataset.id)));
  $$('.batch-delete').forEach(b=>b.addEventListener('click',()=>deleteBatchLocal(b.dataset.id)));
}
...
function sameEstimateAsBatch(est,item){
  if(!est)return false;
  const sameAmount=Math.round(Number(est.amount)||0)===Math.round(Number(item?.amount)||0);
  const src=text(est.source),itemSrc=text(item?.estimateSource);
  return sameAmount&&(!itemSrc||!src||src===itemSrc);
}
function dependentCorrectionBatch(batch){
  const orders=new Set((batch.items||[]).map(i=>i.orderNo).filter(Boolean));
  return state.batches.find(b=>b.status==='active'&&b.batchId!==batch.batchId&&(b.corrections||[]).some(c=>orders.has(c.orderNo)));
}
async function deleteBatchLocal(batchId){
  if(state.busy)return;
  const batch=state.batches.find(x=>x.batchId===batchId&&x.status==='active');
  if(!batch){flash('Batch tidak ditemukan atau sudah dihapus.','error');return;}
  const dependent=dependentCorrectionBatch(batch);
  if(dependent){
    flash(`Batch belum bisa dihapus karena koreksi order dari batch ini sudah dipakai di ${dependent.batchId}. Hapus batch koreksi tersebut lebih dulu.`,'error');
    return;
  }
  const payout=Number(batch.payoutAmount)||Number(batch.baseAmount)+Number(batch.correctionAmount)||0;
  const kindLabel=batch.kind==='estimate'?'Estimasi':batch.kind==='final'?'Final':'Legacy Campuran';
  if(!confirm(`Hapus ${batch.batchId}?\n\nJenis: ${kindLabel}\nPesanan: ${(batch.items||[]).length}\nNominal: ${money(payout)}\n\nStatus order akan dihitung ulang dari data Excel/HTML terbaru. Perubahan hanya lokal sampai Sinkronkan Sekarang ditekan.`))return;
  try{
    state.busy=true;
    const deletedAt=nowIso();
    const idx=state.batches.findIndex(x=>x.batchId===batchId);
    state.batches[idx]=normalizeBatch({...batch,status:'deleted',deletedAt,deletedReason:'manual-delete'},batchId);
    markDirty('batches',batchId);

    for(const item of batch.items||[]){
      const basis=item.basis||batch.kind;
      if(basis==='estimate'){
        const raw=state.rawOrders.get(item.orderNo);
        if(raw){
          const patch={...raw,schemaVersion:SCHEMA_VERSION};
          if(raw.payoutLock?.batchId===batchId)patch.payoutLock=null;
          if(raw.estimateBatchId===batchId)patch.estimateBatchId=null;
          if(raw.correctionSettledBatchId===batchId)patch.correctionSettledBatchId=null;
          if(sameEstimateAsBatch(raw.pendingEstimate,item))patch.pendingEstimate=null;
          if(sameEstimateAsBatch(raw.lastEstimate,item))patch.lastEstimate=null;
          if(Math.round(Number(raw.shopeePendingAmount)||0)===Math.round(Number(item.amount)||0)){
            patch.shopeePendingAmount=0;patch.shopeePendingStatus='';patch.shopeePendingReleaseEstimate='';patch.shopeePendingPaymentMethod='';patch.shopeePendingSourceFile='';patch.shopeePendingImportedAt='';
          }
          patch.estimatePaidAmount=0;patch.estimatePaidAt='';patch.estimatePaidSource='';
          state.rawOrders.set(item.orderNo,patch);markDirty('orders',item.orderNo);
        }
      }
      if(basis==='final'){
        const raw=state.rawIncomes.get(item.orderNo);
        if(raw){
          const patch={...raw,schemaVersion:SCHEMA_VERSION};
          if(raw.payoutBatchId===batchId)patch.payoutBatchId=null;
          if(raw.batchId===batchId)patch.batchId=null;
          if(raw.payoutBatchId===batchId||raw.batchId===batchId)patch.payoutLockedAt='';
          state.rawIncomes.set(item.orderNo,patch);markDirty('incomes',item.orderNo);
        }
      }
    }
    ...
    rebuildState({render:false,save:false});
    rebuildLedgerLocal();
    await logUpload({kind:'Hapus Batch',fileName:batchId,rows:(batch.items||[]).length,summary:`${kindLabel} ${money(payout)} dihapus lokal. Status order dihitung ulang.`});
    rebuildState({render:true,save:true});
    flash(`${batchId} dihapus lokal. Sinkronkan dari Pengaturan bila sudah yakin.`,'success');
  }catch(e){flash(`Gagal menghapus batch: ${e.message}`,'error');}
  finally{state.busy=false;renderSettings();}
}
...
function exportBatches(){const rows=[];for(const b of state.batches.filter(x=>x.status==='active')){for(const i of b.items)rows.push({'Batch':b.batchId,'Waktu':b.createdAt,'Jenis':b.kind,'Jenis Baris':'Pesanan','No. Pesanan':i.orderNo,'Basis':i.basis,'Nominal':i.amount,'Koreksi':'','Total Pencairan':b.payoutAmount||b.baseAmount+b.correctionAmount});for(const c of b.corrections)rows.push({'Batch':b.batchId,'Waktu':b.createdAt,'Jenis':b.kind,'Jenis Baris':'Koreksi','No. Pesanan':c.orderNo,'Basis':'koreksi','Nominal':'','Koreksi':c.appliedAmount,'Total Pencairan':b.payoutAmount||b.baseAmount+b.correctionAmount});}downloadXlsx(rows,`Riwayat_Batch_${today()}.xlsx`,'Batch');}
